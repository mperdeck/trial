...
app.controller('MainController', ['$scope', '$log', '$http',
function ($scope, $log, $http) {
    $scope.$log = $log;
    $scope.message = 'Hello World!';
    $scope.searchParameters = { name: "", code: "" };
    $scope.results = [];
    ...
    $scope.logwarning = function () {
        JL('Angular').warn('jkdkkdkdkd');
    }

    $scope.getSearchResults = function() {
        $http({ method: 'POST', url: '/api/Search', data: $scope.searchParameters, /*timeout: 5000, */warningAfter: 2000 }).
